...
public class ProgramDetail extends JDialog implements ActionListener {
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == saveButton) {
			...
			program.setName(((JTextField) pan[0].getComponent(1)).getText()
					.toString());
			program.setDifficulty(((JTextField) pan[1].getComponent(1))
					.getText().toString());
			program.setPartOfBody(((JTextField) pan[2].getComponent(1))
					.getText().toString());
			program.setComment(((JTextField) pan[3].getComponent(1)).getText()
					.toString());
